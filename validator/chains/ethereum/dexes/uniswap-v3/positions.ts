...
/**
 * Ethereum Uniswap V3 NFT position fetcher
...
export async function getAllNFTPositions(
  ...
  if (!hotkeys || hotkeys.length === 0) {
    logger.info('📝 No hotkeys provided for Ethereum Uniswap V3');
    return [];
  }
  ...
  try {
    ...
    const positionManagerAddress =
      CONFIG.ETHEREUM.UNISWAP_V3_POSITION_MANAGER_ADDRESS;
    ...
    logger.info(
      `[Ethereum/UniswapV3] 🚀 Fetching stakes for ${hotkeys.length} hotkeys using optimized batch method`,
    );
    ...
    logger.info(
      `[Ethereum/UniswapV3] 📦 Processing ${hotkeyChunks.length} chunks of ${HOTKEY_CHUNK_SIZE} hotkeys each`,
    );
    ...
    for (let chunkIdx = 0; chunkIdx < hotkeyChunks.length; chunkIdx++) {
      ...
      logger.info(
        `[Ethereum/UniswapV3] 📦 Processing chunk ${chunkIdx + 1}/${hotkeyChunks.length} (${chunk.length} hotkeys)`,
      );
      ...
      try {
        ...
        logger.info(
          `[Ethereum/UniswapV3] ✅ Chunk ${chunkIdx + 1}/${hotkeyChunks.length} processed: found ${allTokenIds.flat().length} positions`,
        );
      } catch (error: any) {
        ...
        if (
          ...
          logger.info(
            `📝 [Ethereum/UniswapV3] Contract not deployed or chunk ${chunkIdx + 1} failed - continuing with other chunks`,
          );
          // Continue with next chunk instead of failing completely
          continue;
        } else {
          logger.error(
            `❌ [Ethereum/UniswapV3] Failed to process chunk ${chunkIdx + 1}:`,
            error,
          );
    ...
    if (hotkeyTokenPairs.length === 0) {
      logger.info(
        '[Ethereum/UniswapV3] ℹ️ No stakes found for any hotkeys',
      );
      return [];
    }

    logger.info(
      `[Ethereum/UniswapV3] 🎯 Found ${hotkeyTokenPairs.length} total staked positions across ${hotkeys.length} hotkeys`,
    );
    ...
    positionResults.forEach((result: any, idx: number) => {
      ...
      try {
        ...
        if (
          ...
          logger.warn(
            `⚠️ [Ethereum/UniswapV3] Invalid pool address for token ${tokenId}`,
          );
        }
      } catch (error) {
        logger.error(
          `❌ [Ethereum/UniswapV3] Failed to decode position for token ${tokenId}:`,
          error,
        );
      }
    });
    ...
    logger.info(
      `✅ [Ethereum/UniswapV3] Position fetch complete: ${positions.length} positions in ${totalTime}ms`,
    );
    ...
    const totalTime = Date.now() - startTime;
    logger.error(
      `❌ [Ethereum/UniswapV3] Position fetch failed after ${totalTime}ms:`,
      error,
    );
    throw new Error(
      `Ethereum Uniswap V3 position fetch failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
    );
  }
}
