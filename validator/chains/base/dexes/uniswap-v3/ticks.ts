  ...
  try {
    ...
    if (allowedPools) {
      ...
      if (verbose) {
        ...
        logger.info(`[Base] 🔎 allowedPools size: ${size}; sample: ${JSON.stringify(sample)}`);
      } else {
        logger.info(`[Base] 🔎 allowedPools size: ${size}`);
      }
    } else if (verbose) {
      logger.info(`[Base] 🔎 allowedPools not provided; fetching all ticks`);
    }
    ...
    if (allowedPools && allowedPools.size > 0) {
      for (let i = 0; i < poolAddresses.length; i++) {
        ...
        if (allowedPools.has(poolAddress)) {
          ...
          logger.info(`[Base] ⏭️ Skipping pool not in allowed set: ${poolAddress}`);
        }
      }
      logger.info(`[Base] 📋 Filtered to ${filteredPoolData.length} pools from ${poolAddresses.length} total`);
    } else {
      // No filtering - use all pools
      filteredPoolData = poolAddresses.map((address, i) => ({ address, subnetId: subnetIds[i] }));
      logger.info(`[Base] 📋 Processing all ${filteredPoolData.length} pools`);
    }

    if (filteredPoolData.length === 0) {
      logger.info('[Base] ℹ️ No pools to fetch after filtering');
      return {};
    }
    ...
    logger.info(`✅ [Base/UniswapV3] Tick fetch complete:`);
