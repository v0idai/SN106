...
/**
 * Multicall utility for Base chain operations
 * Provides efficient batch RPC calls to reduce network requests
 */

/**
 * Multicall service instance for Base with rate limit handling
 */
class BaseMulticall {
  ...
  constructor() {
    const rpcUrl = CONFIG.BASE.RPC_URL;
    if (!rpcUrl) {
  ...
  async executeBatch(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (isRateLimitError) {
          ...
          logger.warn(
            `⚠️ [Base] Rate limit hit on attempt ${attempt}/${maxRetries}, retrying in ${delay}ms...`,
          );
          await new Promise(resolve => setTimeout(resolve, delay));
          ...
          logger.warn(
            `⚠️ [Base] Multicall attempt ${attempt}/${maxRetries} failed, retrying in ${delay}ms...`,
          );
          await new Promise(resolve => setTimeout(resolve, delay));
        } else {
          // Last attempt failed
          logger.error(
            `❌ [Base] Multicall failed after ${maxRetries} attempts:`,
            error,
          );
        }
      }
    ...
    throw lastError || new Error('Base multicall failed after all retries');
  }
...
// Singleton instance
let multicallInstance: BaseMulticall | null = null;

export function getMulticallInstance(): BaseMulticall {
  if (!multicallInstance) {
    multicallInstance = new BaseMulticall();
  }
...
// Dynamic chunk sizing based on operation type (same as Ethereum)
